...
public class Explorer {
	...
	public Explorer(Sensor sensor, Navigator navigator, MyAIController myAIController) {
		this.sensor = sensor;
	...
	public void explore(float delta, HashMap<Coordinate, MapTile> currentView) {
		...
		if(!sensor.isFollowingWall()){
			navigator.followWall(currentView, delta);
		}
		// Once the car is already stuck to a wall, apply the following logic
		else{
			...
			if(sensor.isTurningRight()){
				...
				if(!sensor.checkFollowingWall(controller.getOrientation(),currentView)){
					...
					controller.getSensor().setTurningLeft(false);
				}
			}
			// Try to determine whether or not the car is next to a wall.
				...
				if(sensor.checkWallAhead(controller.getOrientation(),currentView)){
					...
					controller.getSensor().setTurningRight(true);				
			...
			// This indicates that I can do a left turn if I am not turning right
				...
				controller.getSensor().setTurningLeft(true);		
			}
		}
